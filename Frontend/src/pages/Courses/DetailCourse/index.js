...
import { useCreateReview } from "../../../apis/reviews.api";
import { useCreateNotification } from "../../../apis/notification.api";
import axios from "axios"; // Thêm axios để gọi API gửi email
...
const DetailCourse = () => {
  ...
  const handleRegisterCourse = async () => {
    ...
    // Cập nhật khóa học và thông tin người dùng
    updateCourse({ id, payload: { users: updatedUsers } });
    updateUser({ user: userId, payload: { courses: updataCourses } });
    ...
    try {
      const userEmail = listUsers?.email; // Lấy email từ listUsers
      if (!userEmail) {
        api.warning({
          message: "Không tìm thấy email người dùng",
          description: "Vui lòng kiểm tra thông tin người dùng.",
        });
        return;
      }
      ...
      await axios.post("http://localhost:4000/api/v1/send-email", {
        to: userEmail,
        subject: `Xác nhận đăng ký khóa học: ${listCourses.title || "Khóa học không xác định"}`,
        content: `Chào bạn,\n\nBạn đã đăng ký thành công khóa học "${listCourses.title || "Khóa học không xác định"}".\nHãy bắt đầu học ngay hôm nay!\n\nTrân trọng,\nHệ thống quản lý khóa học`,
      });

      api.success({
        message: "Email xác nhận đã được gửi!",
        description: `Một email xác nhận đã được gửi đến ${userEmail}.`,
      });
    } catch (error) {
      api.error({
        message: "Gửi email thất bại",
        description: error.message || "Không thể gửi email xác nhận.",
      });
    }
  ...
  const handleUnregisterCourse = () => {
    ...
    // Tạo thông báo sau khi hủy đăng ký
    const notificationTitle = `Hủy đăng ký khóa học '${listCourses.title || "Khóa học không xác định"}'`;
    createNotification({
      userId,
      message: notificationTitle,
  ...
  const handleSendReview = () => {
    ...
    createReview(
      ...
      },
      {
        onSuccess: (data) => {
          console.log("data:", data);
          const reviewId = data?.data?.data?._id;
          console.log("reviewId:", reviewId);
          if (reviewId) {
            const updatedReviews = [...(listCourses.reviews || []), reviewId];
